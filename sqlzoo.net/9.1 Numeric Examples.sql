...
# 6. Show the percentage of students who A_STRONGLY_AGREE to question 22 for the subject '(8) Computer Science' show 
#    the same figure for the subject '(H) Creative Arts and Design'.
...
SELECT
 institution
 , SUM(sample)
 , SUM(CASE WHEN subject LIKE '(8)%' AND institution LIKE '%Manchester%' THEN sample ELSE 0 END) AS comp
FROM nss
WHERE
 question='Q01'
 AND institution LIKE '%Manchester%'
