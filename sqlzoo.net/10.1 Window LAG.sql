...
# 2. The LAG function is used to show data from the preceding row or the table. When lining up rows the data is partitioned 
...
SELECT 
 tw.name, 
 DATE_FORMAT(tw.whn,'%Y-%m-%d') AS tw_date, 
 DATE_FORMAT(lw.whn,'%Y-%m-%d') AS lw_date,
 tw.confirmed AS tw_confirmed, 
 lw.confirmed AS lw_confirmed
FROM covid tw 
LEFT JOIN covid lw 
...
SELECT
 name
 ,tw_date
 ,tw_confirmed - lw_confirmed
FROM trial
ORDER BY tw_date ;
------------------------------------------------------------------------------------------------------------------------

# 6. This query shows the number of confirmed cases together with the world ranking for cases for the date '2020-04-20'. The number 
#    of COVID deaths is also shown. United States has the highest number, Spain is number 2...
...
# 7. This query includes a JOIN t the world table so we can access the total population of each country and calculate infection
#    rates (in cases per 100,000). 
...
SELECT 
 name 
 , DATE_FORMAT(whn,'%Y-%m-%d') AS peak_date
...
SELECT
 ...
 , peak_date
 , new_cases
 , ROW_NUMBER() OVER (PARTITION BY name ORDER BY new_cases DESC) AS rn
FROM deneme
...
ORDER BY 2 ;
 ...
 # During this query i recieved an error message which says "Lost connection to MySQL server during query", but in my opinion
 # my answer is right. 
---------------------------------------------------------------------------------------------------------------------------------
